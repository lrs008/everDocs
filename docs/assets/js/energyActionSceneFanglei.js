...
*/ 
function energyActionSceneFanglei(infoList) {
    ...
    var axesHelper = new THREE.AxesHelper( 0.5 );  // for debug 辅助

    var bbox  // for debug 辅助
    ...
    function init() {
        ...
        // loader.load( 'glTF/a2.5_out/a2.5.gltf', function ( gltf ) { VZ200MeshRenamed
        loader.load( '../assets/glTF/BZY100fuben.glb', function ( gltf ) {  // 这个是未压缩的
            // loader.load( '../assets/glTF/VZ200MeshRenamed.glb', function ( gltf ) {  // 这个是未压缩的
            model = gltf.scene;
            // model.rotation.x =  Math.PI/2   // 导入的模型上下颠倒了，所以需要选择转过来
            // 显示gltf.scene的Boundbox 
            model.traverse( function ( object ) {
                // console.log("object.type,object.name = ",object.type,object.name)
                if ( object.type === 'Mesh' ) {
            ...
            if(object.type === 'Group' && object.name !== ""){
                object.traverse( function ( mesh ) {
                    setMeshMaterial(mesh,0x808080);   // 黑色弯管
                } )

                // console.log("object.type,object.name = ",object.type,object.name)
                // console.log(object)
                object.userData.name = "暂无名字"
                object.userData.desc = " 该部件暂无描述该部件暂无描述该部件暂无描述该部件暂无描述该部件暂无描述该部件暂无描述该部件暂无描述"
                if(object.name === '584'){
                    object.userData.name ="排气门"
                    object.userData.desc = " xx "
                    object.traverse( function ( mesh ) {    
                        setMeshMaterial(mesh,0xE08E79);   
                    } )
                }
                if(object.name === '583'){        
                    object.userData.name ="燃烧机"
                    object.userData.desc = " xx "
                    object.traverse( function ( mesh ) {
                ...
                if(object.name === '394'){
                    object.userData.name ="冷却水出口"
                    object.userData.desc = " xx "
                    object.traverse( function ( mesh ) {   
                        setMeshMaterial(mesh,0x182C7C);   
                    } )
                }
                if(object.name === '375'){           
                    object.userData.name ="冷水出口"
                    object.userData.desc = " xx "
                    // const animate = () => {
                ...
                if(object.name === '392'){
                    object.userData.name ="冷水入口"
                    object.userData.desc = " xx "
                    object.traverse( function ( mesh ) {   
                        setMeshMaterial(mesh,0x2C93E8);    
                    } )
                }
                if(object.name === '396'){
                    object.userData.name ="冷却水入口"
                    object.userData.desc = " xx "
                    object.traverse( function ( mesh ) {  
                        setMeshMaterial(mesh,0x182C7C);   
                    } )
                ...
                if(object.name === '595'){
                    object.userData.name ="稀释液角阀"
                    object.userData.desc = " xx "
                    object.traverse( function ( mesh ) {
                        setMeshMaterial(mesh,0x182C7C);   // 正方体小盖子第二层 白色   6.128882864	w.s
                    } ) 
                }
                if(object.name === '613'){
                    object.userData.name ="浓溶液角阀"
                    object.userData.desc = " xx "
                    object.traverse( function ( mesh ) {
                        setMeshMaterial(mesh,0xE08E79);   // 底板2  黄
                    } )
                }
    ...
    ***************************************************************/ 
    function onDocumentMouseDown(event) {
        ...
        if (intersects.length > 0) {
            ...
            // console.log("######### onDocumentMouseDown selectedObject.parent = ")
   ...
   function onDocumentTouchstart(e){
    ...
    function addDivOnGroup(object) {
        ...
        objnameP.innerHTML = object.userData.name
        ...
        // // object.userData.name = 'shwfeihwieh'
        // // console.log(object.userData.name)
        // console.log(object)

        CSS2DObject.position.set((xmin + xmax)/2,0.15,0);  // 这里的坐标是相对于object的坐标
    ...
    */ 
    function processAttrShow(intersects){
		...
		if (intersects.length < 1) {
            ...
            // console.log(object)
            infoList.push({key:'模型序号',value: object.name})
            infoList.push({key:'部件名称',value: object.userData.name})
            infoList.push({key:'部件描述',value: object.userData.desc})
        }
    }
